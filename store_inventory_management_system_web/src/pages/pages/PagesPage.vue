  ...
  <div class="space-y-4">
    ...
    <el-dialog v-model="visible" :title="form.id ? '编辑页面' : '新建页面'" width="640px">
      <el-form :model="form" label-width="110px">
        <el-form-item label="父页面ID"><el-input v-model.number="form.parentId" /></el-form-item>
        ...
        <el-form-item label="排序"><el-input-number v-model.number="form.orderNum" :min="0" /></el-form-item>
    ...
    <el-dialog v-model="permVisible" title="页面权限设置" width="520px">
      <div class="space-y-3">
        ...
        <div class="flex items-center space-x-2">
          <el-select v-model="newPermId" placeholder="选择单个权限" style="width: 280px">
            <el-option v-for="p in permList" :key="p.id" :value="p.id" :label="p.name" />
          </el-select>
          <el-button @click="addOne" :disabled="!currentPageId || !newPermId">新增</el-button>
        </div>
      </div>
      <template #footer>
...
const form = reactive<any>({ id: 0, parentId: 0 as number, name: '', path: '', component: '', icon: '', orderNum: 0, visible: 1, meta: '' })
...
async function fetch() {
  loading.value = true
  const res = await http.get('/page/list', { params: { current: page.current, size: page.size } })
...
async function savePerms() {
  ...
  await http.put(`/page/${currentPageId.value}/permission`, { permissionIds: assignIds.value })
