  ...
  <div class="space-y-4 p-4">
    ...
    >
      ...
      <template #actions="{ row }">
        <el-button plain round type="primary" @click="openDetail(row)"
          >详情</el-button
        >
        <el-button
          v-if="row.status === 0 && canPay(row)"
          plain
          round
          type="success"
          @click="onPay(row.id)"
    ...
    <el-dialog v-model="detailVisible" title="订单详情" width="600px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="金额单ID">{{
          detail?.amountOrderId
        }}</el-descriptions-item>
        ...
        <el-descriptions-item label="状态">
          <el-tag v-if="detail?.amountOrderStatus === 0" type="danger"
            >待支付</el-tag
          >
          <el-tag v-else-if="detail?.amountOrderStatus === 1" type="success"
            >已支付</el-tag
          >
          <el-tag v-else-if="detail?.amountOrderStatus === 2" type="info"
            >已取消</el-tag
          >
        </el-descriptions-item>
        <el-descriptions-item label="支付方式">{{
          detail?.payType === "alipay" ? "支付宝" : detail?.payType
        }}</el-descriptions-item>
        <el-descriptions-item label="付款方ID">{{
          detail?.payerId
        }}</el-descriptions-item>
        <el-descriptions-item label="收款方ID">{{
          detail?.storeId
        }}</el-descriptions-item>
      ...
      <div v-if="detail?.productName" class="mt-4">
        ...
        <div class="flex gap-4 items-center bg-gray-50 p-2 rounded">
          ...
          <div>
            ...
            <div class="text-sm">
              单价: ¥{{ detail.productPrice }} x {{ detail.productQuantity }}
            </div>
          </div>
        </div>
      </div>
...
function canPay(row: any) {
  ...
  if (currentUser && String(row.payerId) === String(currentUser.id))
    return true;
  ...
  const isManager = roles.some(
    (r: any) => r.roleName === "门店管理员" || r.roleName === "超级管理员",
  );
