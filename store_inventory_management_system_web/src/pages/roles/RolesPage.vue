<template>
  ...
  <vxe-table :data="rows" :loading="loading" border stripe>
    ...
    <vxe-column title="操作" width="240">
      <template #default="scope">
        ...
        <el-button link type="primary" @click="openAssignPages(scope.row)"><el-icon><Edit /></el-icon>页面分配</el-button>
          <el-button link type="primary" @click="openAssignPerms(scope.row)"><el-icon><Key /></el-icon>权限分配</el-button>
        <el-button v-perm="'角色管理'" link type="danger" @click="remove(scope.row)"><el-icon><Delete /></el-icon>删除</el-button>
      </template>
    </vxe-column>
  </vxe-table>
  <vxe-pager class="mt-3 flex justify-end" :total="page.total" :current-page="page.current" :page-size="page.size" @page-change="onPager" />
  <el-dialog v-model="visible" :title="form.id ? '编辑角色' : '新建角色'" width="520px">
    <el-form :model="form" label-width="110px">
      <el-form-item label="角色名称"><el-input v-model="form.roleName" /></el-form-item>
      <el-form-item label="角色描述"><el-input v-model="form.description" /></el-form-item>
    </el-form>
    <template #footer>
  ...
  <el-dialog v-model="assignPageVisible" title="给角色分配页面" width="520px">
    <el-checkbox-group v-model="assignPageIds">
      <el-checkbox v-for="p in pageList" :key="p.id" :label="p.id">{{p.name}}</el-checkbox>
    </el-checkbox-group>
    <template #footer>
  ...
  <el-dialog v-model="assignPermVisible" title="给角色分配权限" width="520px">
    <el-checkbox-group v-model="assignPermIds">
      <el-checkbox v-for="p in permList" :key="p.id" :label="p.id">{{p.name}}</el-checkbox>
    </el-checkbox-group>
    <template #footer>
...
async function fetch() {
  loading.value = true
  const res = await http.get('/role/list', { params: { current: page.current, size: page.size } })
  rows.value = res.data.records
  page.total = res.data.total
  loading.value = false
}
function onPager(e: any) { page.current = e.currentPage; page.size = e.pageSize; fetch() }
function openCreate() { Object.assign(form, { id: 0, roleName: '', description: '' }); visible.value = true }
...
async function remove(row: any) { await http.delete(`/role/${row.id}`); fetch() }
async function openAssignPages(row: any) {
  currentRoleId.value = row.id
  const resPages = await http.get('/page/list', { params: { current: 1, size: 200 } })
...
async function assignPages() {
  ...
  await http.post(`/role/${currentRoleId.value}/assign-pages`, { pageIds: assignPageIds.value })
  assignPageVisible.value = false
}
async function openAssignPerms(row: any) {
  currentRoleId.value = row.id
  const resPerms = await http.get('/permission/list', { params: { current: 1, size: 200 } })
...
async function assignPerms() {
  if (!currentRoleId.value) return
  await http.put(`/role/${currentRoleId.value}/permission`, { permissionIds: assignPermIds.value })
