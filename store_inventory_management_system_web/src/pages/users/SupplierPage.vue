<template>
  <div class="space-y-4">
    <div class="flex justify-between">
      <div class="text-lg font-medium">供应商管理</div>
      <el-button type="primary" @click="openCreate" v-perm="'user:add'"
        ><el-icon><Plus /></el-icon><span>新建供应商</span></el-button
      >
    </div>
    <EpTable
      :rows="rows"
...
async function fetch() {
  ...
  const res = await http.get("/user/list", {
    params: {
      ...
      roleName: targetRoleName, // 过滤供应商角色
    },
  });
...
async function submit() {
  ...
  if (form.id) {
    // 编辑时，保留原有角色并确保有 Supplier 角色（简单起见，这里覆盖为 Supplier，或者先查询？）
    // 为了简单，我们只更新基本信息，或者强制设置为 Supplier
    // 后端 updateUser 会覆盖角色列表，所以这里必须传 roleIds
