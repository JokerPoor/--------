<template>
  <div class="space-y-4">
    <div class="flex justify-between">
      ...
      <el-button type="primary" @click="openCreate"><el-icon><Plus /></el-icon>新建权限</el-button>
    </div>
    <vxe-table :data="rows" :loading="loading" border stripe>
      ...
      <vxe-column title="操作" width="200">
        <template #default="scope">
          <el-button v-perm="'权限分配'" link type="primary" @click="openEdit(scope.row)"><el-icon><Edit /></el-icon>编辑</el-button>
          <el-button v-perm="'权限分配'" link type="danger" @click="remove(scope.row)"><el-icon><Delete /></el-icon>删除</el-button>
        </template>
      </vxe-column>
    </vxe-table>
    ...
    <el-dialog v-model="visible" :title="form.id ? '编辑权限' : '新建权限'" width="520px">
      <el-form :model="form" label-width="110px">
        ...
        <el-form-item label="权限描述"><el-input v-model="form.description" /></el-form-item>
      </el-form>
      <template #footer>
...
async function fetch() {
  loading.value = true
  const res = await http.get('/permission/list', { params: { current: page.current, size: page.size } })
  rows.value = res.data.records
  page.total = res.data.total
  loading.value = false
}
function onPager(e: any) { page.current = e.currentPage; page.size = e.pageSize; fetch() }
function openCreate() { Object.assign(form, { id: 0, name: '', description: '' }); visible.value = true }
...
async function remove(row: any) { await http.delete(`/permission/${row.id}`); fetch() }
fetch()
</script>
