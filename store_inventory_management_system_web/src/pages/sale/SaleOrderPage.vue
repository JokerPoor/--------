  ...
  <div class="p-4 bg-white">
        ...
        >
          ...
          <template #actions="{ row }">
            <el-button
              ...
              round
              type="success"
              @click="onConfirmArrival(row)"
              v-perm="'sale:order:confirm'"
              >确认收货</el-button
            >
            <el-button
              v-if="row.status === 2"
        ...
        >
          ...
          <template #actions="{ row }">
            <el-button
              ...
              type="primary"
              @click="onShip(row)"
              v-perm="'inventory:sale-order:ship'"
              >发货</el-button
            >
          </template>
        </EpTable>
    ...
    <el-dialog v-model="createVisible" title="购买商品" width="800px">
      <div class="mb-4 flex gap-2">
        ...
        >
          <template #append
            ><el-button @click="searchInventory"
      ...
      <el-table
        ...
        @current-change="onInventorySelect"
      >
        <el-table-column width="50">
          <template #default="{ row }">
            <el-radio :model-value="selectedInventory?.id" :label="row.id">{{
              ""
            }}</el-radio>
          </template>
        </el-table-column>
      ...
      <div class="mt-4 flex items-center gap-4 justify-end">
        ...
        />
        <div class="text-red-500 font-bold w-32 text-right">
          总价: ¥{{ totalPrice }}
        </div>
      </div>
...
const hasManagePerm = computed(() =>
  auth.hasPermission("inventory:sale-order:ship"),
);
...
async function onShip(row: any) {
  ...
  await http.post("/inventory/sale-order", row.id, {
