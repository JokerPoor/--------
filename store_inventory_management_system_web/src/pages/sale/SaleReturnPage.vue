        ...
        >
          ...
          <template #actions="{ row }">
            <el-button
              ...
              type="success"
              @click="onConfirm(row)"
              v-perm="'inventory:sale-return:confirm'"
              >确认退货</el-button
            >
          </template>
        </EpTable>
...
const hasManagePerm = computed(() =>
  auth.hasPermission("inventory:sale-return:confirm"),
);
...
async function onConfirm(row: any) {
  await ElMessageBox.confirm("确认收到退货并入库?", "提示", {
    type: "warning",
  });
  // Use plain post body for Long
  await http.post("/inventory/sale-return/confirm", row.id, {
