  ...
  <div class="space-y-6 p-4">
    ...
    >
      ...
      >
        ...
        <div class="p-4 flex-1 flex flex-col">
          <h3
            class="text-lg font-bold text-gray-800 mb-1 truncate"
            :title="item.productName"
          >
            {{ item.productName }}
          </h3>
          <p class="text-gray-500 text-xs mb-3 line-clamp-2 h-8">
            {{ item.productDescription || "暂无描述" }}
          </p>
          ...
          <div class="mt-auto">
            ...
            <div class="flex gap-2">
              <el-button class="flex-1" @click="openDetail(item)"
                >详情</el-button
              >
              <el-button
                type="primary"
    ...
    >
      <div v-if="detailProduct" class="flex flex-col md:flex-row gap-6">
        <div class="w-full md:w-1/2">
          <img
            :src="
              detailProduct.productUrl || 'https://via.placeholder.com/300x300'
            "
        ...
        <div class="w-full md:w-1/2 flex flex-col">
          ...
          <div class="flex items-center gap-2 mb-4">
            <span class="text-2xl font-bold text-red-600"
              >¥{{ detailProduct.productPrice }}</span
            >
            <el-tag :type="detailProduct.quantity > 0 ? 'success' : 'danger'">
          ...
          <div class="flex-1 overflow-y-auto max-h-48 mb-4">
            ...
            >
              {{ detailProduct.productDescription || "暂无详细描述" }}
            </p>
          </div>

          <div class="mt-auto pt-4 border-t border-gray-100">
            <p class="text-xs text-gray-400 mb-2">
              店铺: {{ detailProduct.storeName || "未知门店" }}
            </p>
            <el-button
              type="primary"
              class="w-full"
              @click="openBuyFromDetail"
    ...
    >
      <div v-if="selectedProduct" class="text-center">
        ...
        <p class="text-red-500 text-xl font-bold my-2">
        ...
        <p class="text-gray-400 text-xs mt-2">
          剩余库存: {{ selectedProduct.quantity }}
        </p>

        <div
          class="bg-gray-50 p-3 rounded-lg mt-6 flex justify-between items-center"
        >
          <span>总计金额:</span>
          <span class="text-xl font-bold text-red-600"
            >¥{{
              (selectedProduct.productPrice * buyQuantity).toFixed(2)
            }}</span
          >
        </div>
      </div>
