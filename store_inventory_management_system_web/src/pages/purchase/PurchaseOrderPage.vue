  ...
  <div>
    ...
    >
      ...
      <template #type="{ row }">
        ...
        <el-tag v-else-if="row.purchaseOrderType === 1" type="warning"
          >自动</el-tag
        >
      ...
      <template #status="{ row }">
        ...
        <el-tag v-else-if="row.purchaseOrderStatus === 1" type="warning"
          >已发货</el-tag
        >
        <el-tag v-else-if="row.purchaseOrderStatus === 2" type="success"
          >已入库</el-tag
        >
      </template>
      ...
      <template #payStatus="{ row }">
        ...
        <el-tag v-else-if="row.amountOrderStatus === 1" type="success"
          >已支付</el-tag
        >
        <el-tag v-else-if="row.amountOrderStatus === 2" type="info"
          >已取消</el-tag
        >
      </template>
      ...
      <template #actions="{ row }">
        <el-button
          v-if="row.amountOrderStatus === 0"
          plain
          round
          type="primary"
          @click="onPay(row.amountOrderId)"
          v-perm="'POST:/amount/order/payorder/:id'"
          >付款</el-button
        >
        <el-button
          v-if="row.amountOrderStatus === 0"
          ...
          round
          type="warning"
          @click="onMockPay(row.amountOrderId)"
          >一键支付</el-button
        >
        <el-button
        ...
        >
        <el-button
          v-if="row.purchaseOrderStatus === 1"
          plain
          round
          type="success"
          @click="openStockIn(row)"
          v-perm="'purchase:order:stock-in'"
          >入库</el-button
        >
        <el-button
    ...
    <el-dialog v-model="stockInVisible" title="入库分配" width="600px">
      <div class="mb-4">
        <div class="font-bold text-lg mb-2">
          订单商品: {{ currentOrder?.productName }}
        </div>
        <div class="flex gap-4 text-sm text-gray-600">
          <span>待入库总数: {{ currentOrder?.productQuantity }}</span>
          <span
            :class="{
              'text-green-500': remainingQty === 0,
              'text-red-500': remainingQty !== 0,
            }"
          >
            剩余分配: {{ remainingQty }}
          </span>
      ...
      <div
        v-for="(item, idx) in stockInItems"
        :key="idx"
      ...
      >
        ...
        <el-button
          type="danger"
          circle
          @click="stockInItems.splice(idx, 1)"
      ...
      <el-form :model="returnForm" label-width="100px">
        ...
        <el-form-item label="退货数量" required>
          <el-input-number
            ...
            :min="1"
            :max="currentOrder?.productQuantity"
            style="width: 100%"
          />
        </el-form-item>
...
async function searchProducts(query: string) {
  ...
  try {
    const res = await http.get("/product/list", {
      params: { name: query, current: 1, size: 20 },
    });
...
async function onMockPay(id: number) {
  if (!id) return ElMessage.error("订单异常，缺少金额单ID");
  try {
    await ElMessageBox.confirm("确定使用模拟支付直接完成订单？", "一键支付", {
      type: "warning",
    });
...
async function openStockIn(row: any) {
  ...
  stockInItems.value = [
    { warehouseId: undefined, quantity: row.productQuantity },
  ];
...
const remainingQty = computed(() => {
  ...
  const allocated = stockInItems.value.reduce(
...
async function submitStockIn() {
  if (remainingQty.value !== 0)
    return ElMessage.warning("分配数量必须等于订单总数");
  if (stockInItems.value.some((x) => !x.warehouseId))
