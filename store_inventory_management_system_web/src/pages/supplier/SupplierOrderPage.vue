<template>
  <div class="space-y-4 p-4">
    ...
    >
      ...
      <template #status="{ row }">
        <el-tag v-if="row.purchaseOrderStatus === 0" type="danger"
          >待发货</el-tag
        >
      ...
      <template #actions="{ row }">
        ...
        <span
          v-else-if="
            row.purchaseOrderStatus === 0 && row.amountOrderStatus !== 1
          "
    ...
    <!-- Ship Dialog -->
    <el-dialog v-model="shipVisible" title="确认发货" width="400px">
      <div class="text-center py-4">
        <p class="text-gray-600">
          确定要对订单
          <strong>{{ currentOrder?.productName }}</strong> 进行发货操作吗？
        </p>
...
async function confirmShip() {
  ...
  try {
    await http.post(
      `/purchase/order/ship/${currentOrder.value.purchaseOrderId}`,
    );
